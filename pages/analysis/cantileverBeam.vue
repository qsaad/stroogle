                    ...
                    <v-card-text class="text-xs-center pa-0 ma-0">
                        <v-stage :config="{width:width, height:height}">
                            <v-layer ref="layer">
                                ...
                                <konva-text :xo="0.40*width" :yo="0.52*height" :text="L" :unit="'ft'" v-if="L > 0"></konva-text>
                                <!-- <konva-label :xo="0.5*width" :yo="0.51*height" :text="L" :unit="'ft'" :name="'L'" :pointer="'up'" @selected-text="edit"></konva-label> -->
            
                                 <!-- RIGHT REACTIONS -->
                                <!-- <konva-support :xo="0.9*width" :yo="0.55*height" :radius="10" :rotation="0"></konva-support> -->
                                ...
                                <!-- +++++++++++++++++++++++++++++++++++++++++ -->
                                <!-- MOMENT -->
                                <konva-text :xo="0.75*width" :yo="0.64*height" symbol="M" :text="Mc" unit="k-ft" ></konva-text>
                                ...
                                <!-- DEFLECTION -->
                                <konva-text :xo="0.15*width" :yo="0.64*height" symbol="D" :text="Dc" unit="in" ></konva-text>
    ...
    export default {
        ...
        computed:{
            ...
            Di(){
                ...
                _.forEach(this.Lx(), (x,i) =>{
                    Dx = this.w/(24*this.E*this.Ix())*1728*(Math.pow(x,4)-4*Math.pow(this.L,3)*x+3*Math.pow(this.L,4))
                    _.forEach(this.PL, (Pi,j)=>{
                        let a = Pi.a
                        let b = this.L - a
                        let P = Pi.P
                        if(x <= a){
                            Dx =  Dx + (P*b*b*1728)/(6*this.E*this.Ix()*this.L)*(3*this.L-3*x-b)
                        }//IF
                        else{
                            Dx =  Dx + (P*Math.pow(this.L - x,2)*1728)/(6*this.E*this.Ix()*this.L)*(3*b-this.L+x)
                        }//ELSE
                    })//P LOOP
                    D.push(Dx)
                })//X LOOP
